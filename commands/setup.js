...
module.exports = {
  ...
  options: [
    {
      name: "channel",
      description:
        "Select the channel where you want to send welcome messages.",
    ...
    try {
      if (!interaction.member.permissions.has("ADMINISTRATOR")) {
        return interaction.reply({
          content:
            "You need to be a server administrator to set up the welcome message system.",
          ephemeral: true,
        });
      }
      ...
      await interaction.reply({
        content: "An error occurred while setting up welcome messages.",
        ephemeral: true,
      });
    }
  },
  async sendWelcomeMessage(member, footerIconURL) {
    try {
      ...
      const welcomeMessageDm = `<a:taj:859462735691382784>  **This message has been sent from** **<@725620582942507065>**`;
      ...
      const welcomeChannel = await member.guild.channels.fetch(
        guildSetupData.welcomeChannelId,
      );
